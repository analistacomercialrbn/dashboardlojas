...
@st.cache_data
def load_city_ref():
    return pd.read_csv("supervisao/municipios_centroides.csv", dtype={"CD_MUN":str})
...
st.markdown(f"""
<div class='brandbar'>
  <div><div class='brand-title'>Dashboard de Supervisão</div><div class='brand-sub'>Gestão comercial • faturamento, carteira, mix e cobertura geográfica</div></div>
...
aba1,aba2,aba3,aba4 = st.tabs(["Visão Geral","Carteira","Mix e Oportunidades","Cidades"])
...
with aba4:
    st.subheader("Detalhe por cidade")
    st.markdown("<div class='section-note'>Clique em uma cidade no mapa para abrir o detalhe comercial. O seletor abaixo também funciona como alternativa.</div>",unsafe_allow_html=True)

    cidade_col = first_col(clientes, ["CIDADE","MUNICIPIO","MUNICÍPIO"])
    uf_col = first_col(clientes, ["UF","ESTADO","SIGLA_UF"])

    if not cidade_col or not uf_col:
        st.warning("Não encontrei as colunas de cidade/UF na aba CLIENTES.")
    elif fat.empty:
        st.info("Sem faturamento para o recorte selecionado.")
    else:
        cli_loc = clientes[["CODCLI",cidade_col,uf_col]].drop_duplicates("CODCLI").rename(columns={cidade_col:"CIDADE",uf_col:"UF"})
        loc = fat.merge(cli_loc,on="CODCLI",how="left")
        loc["CIDADE_N"] = loc["CIDADE"].map(norm)
        loc["UF_N"] = loc["UF"].map(norm)

        city = loc.groupby(["CIDADE","UF","CIDADE_N","UF_N"],dropna=False).agg(
            FATURAMENTO=("VALOR","sum"), CLIENTES=("CODCLI","nunique"), PEDIDOS=("NUMPED","nunique"),
            PRODUTOS=("CODPROD","nunique")
        ).reset_index()
        pc_city = loc.groupby(["CIDADE_N","UF_N","CODCLI"]).CODPROD.nunique().rename("MIXCLI").reset_index()
        mix_city = pc_city.groupby(["CIDADE_N","UF_N"]).MIXCLI.mean().rename("MIX").reset_index()
        city = city.merge(mix_city,on=["CIDADE_N","UF_N"],how="left")

        geo = load_city_ref()
        geo["CIDADE_N"] = geo.CIDADE.map(norm); geo["UF_N"] = geo.UF.map(norm)
        city = city.merge(geo[["CIDADE_N","UF_N","LAT","LON"]],on=["CIDADE_N","UF_N"],how="left")
        ...
        z2.markdown(kpi("Maior cidade",city.loc[city.FATURAMENTO.idxmax(),"CIDADE"] if len(city) else "—","Por faturamento"),unsafe_allow_html=True)
        z3.markdown(kpi("Faturamento cidades",brl_compacto(city.FATURAMENTO.sum()),"Recorte atual"),unsafe_allow_html=True)
        z4.markdown(kpi("Cidades no mapa",nint(mapped.shape[0]),"Com coordenadas localizadas"),unsafe_allow_html=True)

        selected_label = None
        if not mapped.empty:
            mapped["LABEL"] = mapped["CIDADE"].astype(str)+" - "+mapped["UF"].astype(str)
            fig = px.scatter_mapbox(
                mapped, lat="LAT", lon="LON", size="FATURAMENTO", color="FATURAMENTO",
                hover_name="LABEL", hover_data={"FATURAMENTO":":,.2f","CLIENTES":True,"PEDIDOS":True,"MIX":":.2f","LAT":False,"LON":False},
                custom_data=["LABEL"], zoom=4.7, height=590,
                color_continuous_scale=[[0,"#DDE2F4"],[0.45,"#59659A"],[1,NAVY]],
                size_max=34
            )
            fig.update_layout(mapbox_style="carto-positron",margin=dict(l=0,r=0,t=10,b=0),coloraxis_colorbar_title="Faturamento")
            try:
                ...
                if pts:
                    ...
                    if cd: selected_label = cd[0] if isinstance(cd,(list,tuple)) else cd
            except TypeError:
                st.plotly_chart(fig,use_container_width=True)

        labels = sorted((city["CIDADE"].astype(str)+" - "+city["UF"].astype(str)).unique())
        idx = labels.index(selected_label) if selected_label in labels else None
        choice = st.selectbox("Cidade para detalhar",labels,index=idx,placeholder="Clique no mapa ou selecione a cidade")
        if choice:
            ...
            a5.markdown(kpi("Mix médio",dec(dcli.PRODUTOS.mean()),"Produtos por cliente"),unsafe_allow_html=True)
            ...
            with c2:
                ...
                fig = px.bar(dep_city,x="VALOR",y="DEPARTAMENTO",orientation="h",title=f"Departamentos — {cname}")
                fig.update_traces(marker_color=NAVY_2)
                st.plotly_chart(chart_layout(fig,390,"v"),use_container_width=True)

            st.subheader("Clientes da cidade")
            detail = dcli.sort_values("FATURAMENTO",ascending=False).copy()
            ...
            st.dataframe(detail[["CODCLI","Faturamento","Mix produtos","Pedidos"]].rename(columns={"CODCLI":"Código cliente"}),use_container_width=True,hide_index=True)
