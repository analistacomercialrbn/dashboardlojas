...
def _filtrar_departamentos(data, deps, incluir_outros):
    ...
    acumulado_extra = {'mensal':{}, 'percentual_mensal':{}}
    for nome in list(deps.keys()):
        if _normalizar(nome) not in validos_norm:
            ...
            for mk,mv in (rec.get('mensal') or {}).items():
                acumulado_extra['mensal'][str(mk)] = acumulado_extra['mensal'].get(str(mk),0.0)+_num(mv)
            deps.pop(nome,None)

    if incluir_outros:
        row = _linha_vazia(OUTROS)
        rec = deps.setdefault(OUTROS,{})
        if not rec.get('mensal') and acumulado_extra['mensal']:
            rec['mensal'] = acumulado_extra['mensal']
        linhas.append(row)
    else:
...
def aplicar_departamentos_unificados():
    """Departamentos são definidos por participação percentual mensal da meta do supervisor."""
    st.markdown(
        """
        <style>
        ...
        .gm-du-head{display:grid;grid-template-columns:2fr repeat(3,.78fr 1.15fr) 1.15fr .9fr;gap:9px;padding:0 12px 7px;align-items:end}.gm-du-head span{font-size:8px;color:#9298a6;text-transform:uppercase;font-weight:850;text-align:center}.gm-du-head span:first-child{text-align:left}
        ...
        @media(max-width:1100px){.gm-du-head{display:none}.gm-du-summary,.gm-du-total{grid-template-columns:1fr}.gm-du-panel-meta{text-align:left}}
        </style>
        """,
    ...
    def data_editor(data=None,*args,**kwargs):
        ...
        incluir_outros=st.toggle('Incluir Outros',value=outros_existente,key=f'gm_du_outros_{key}_{sup}',help='Reserva parte da participação para departamentos sem meta formal.')
        saida=_filtrar_departamentos(data,deps,incluir_outros)

        st.markdown(f"<div class='gm-du-panel'><div class='gm-du-panel-top'><div><div class='gm-du-panel-title'>Participação dos departamentos</div><div class='gm-du-panel-sub'>Informe a % de cada departamento em cada mês. O valor em R$ é calculado sobre a meta mensal do supervisor.</div></div><div class='gm-du-panel-meta'><span>Meta do supervisor</span><strong>{_fmt(meta_sup)}</strong></div></div></div>",unsafe_allow_html=True)

        st.markdown("<div class='gm-du-head'><span>Departamento</span>"+''.join(f"<span>{gm.MESES[m]} %</span><span>{gm.MESES[m]} R$</span>" for m in meses)+"<span>Meta total</span><span>Status</span></div>",unsafe_allow_html=True)
        ...
        for idx,row in saida.reset_index(drop=True).iterrows():
            ...
            mensal=rec.setdefault('mensal',{}); pct=rec.setdefault('percentual_mensal',{})
            ...
            with st.container(border=True):
                ...
                with cols[0]:
                    sub='Opcional • sem meta formal' if dep==OUTROS else f'Referência histórica: {part_ref:.1f}%'
                    st.markdown(f"<div class='gm-du-name'>{dep}</div><div class='gm-du-sub'>{sub}</div>",unsafe_allow_html=True)

                pos=1
                for m in meses:
                    with cols[pos]:
                        novo=st.number_input(f'{gm.MESES[m]} % • {dep}',min_value=0.0,max_value=100.0,value=_num(pct.get(str(m))),step=1.0,format='%.2f',key=f'gm_du_pct_{key}_{idx}_{m}',label_visibility='collapsed')
                    pct[str(m)]=float(novo); pct_somas[str(m)]+=float(novo)
                    mensal[str(m)]=_num(sup_mensal.get(str(m)))*float(novo)/100.0
                    with cols[pos+1]:
                        st.markdown(f"<div class='gm-du-val'><strong>{_fmt(mensal[str(m)])}</strong><span>calculado</span></div>",unsafe_allow_html=True)
                    pos+=2

                rec['mensal']={str(m):_num(mensal.get(str(m))) for m in meses}
                rec['proposta']=sum(rec['mensal'].values())
                saida.loc[saida.index[idx],'Meta proposta']=rec['proposta']
                with cols[-2]:
                    st.markdown(f"<div class='gm-du-val'><strong>{_fmt(rec['proposta'])}</strong><span>total</span></div>",unsafe_allow_html=True)
                with cols[-1]:
                    st.markdown("<div class='gm-du-status ok'>Calculado<br>por %</div>",unsafe_allow_html=True)
        ...
        for m in meses:
            ...
            soma_valor=sum(_num((deps.get(str(r['Departamento']),{}).get('mensal') or {}).get(str(m))) for _,r in saida.iterrows())
            ok=(abs(soma_pct-100)<=0.01) if alvo>0 else abs(soma_pct)<=0.01
            meses_ok=meses_ok and ok
            cards.append(f"<div class='gm-du-box'><span>{gm.MESES[m]}</span><strong>{soma_pct:.1f}%</strong><small>{_fmt(soma_valor)} de {_fmt(alvo)} • {'Fechado' if ok else 'Ajustar para 100%'}</small></div>")
        if cards: st.markdown("<div class='gm-du-summary'>"+''.join(cards)+"</div>",unsafe_allow_html=True)

        tudo_ok=meses_ok and abs(dif_total)<=0.02
        estado['ok'][sup]=tudo_ok
        if tudo_ok: st.success('Participação dos departamentos fecha 100% em todos os meses.')
        else: st.warning('A soma das participações dos departamentos precisa fechar 100% em cada mês.')
        return saida
