...
st.set_page_config(page_title='Logística Operacional | RBN', page_icon='🚚', layout='wide')
st.markdown('''<style>
...
def source():
    ...
    for p in ['data/FORMACOES_DE_CARGA.xlsx','FORMAÇÕES DE CARGA.xlsx','FORMAÇÕES DE CARGA']:
        if os.path.exists(p): return p
    raise FileNotFoundError('Configure DATA_XLSX_URL em Secrets.')

@st.cache_data(ttl=300,show_spinner=False)
def load():
    src=source(); raw=pd.read_excel(src,sheet_name='FORMAÇÃO 2026',header=1,engine='openpyxl'); raw.columns=[norm(c) for c in raw.columns]
    ...
    try:
        src2=source(); rg=pd.read_excel(src2,sheet_name='CADASTRO REGIAO',header=0,engine='openpyxl'); rg.columns=[norm(c) for c in rg.columns]
        mc=next((c for c in rg.columns if 'MUNICIPIO' in c),None); rc=next((c for c in rg.columns if 'REGIAO DE PLANEJAMENTO' in c),None)
        if mc and rc:
            rg=rg[[mc,rc]].dropna(); rg['KEY']=rg[mc].astype(str).str.upper().str.strip(); d['KEY']=d['CIDADE'].fillna('').astype(str).str.upper().str.strip(); d=d.merge(rg[['KEY',rc]].drop_duplicates('KEY'),on='KEY',how='left').rename(columns={rc:'REGIAO'})
...
try: df=load()
    ...
    st.error('Fonte não configurada'); st.info('Defina DATA_XLSX_URL no Streamlit Secrets com um link de download/exportação somente leitura.'); st.code(str(e)); st.stop()
...
if page=='Visão Geral':
    aberto=f[~f.ENTREGUE]
    cards([('Registros em aberto',br(len(aberto)),'unidades operacionais'),('Peso em aberto',f'{br(aberto.PESO.sum()/1000,1)} t',''),('Em rota',br(f.EM_ROTA.sum()),''),('Entregues',br(f.ENTREGUE.sum()),''),('Espera média',f'{br(aberto.DIAS_ESPERA.mean(),1)} dias',''),('Ocorrências',br(f.TEM_OCORRENCIA.sum()),'')])
    ...
    st.markdown('<div class="section">Mapa operacional — dados preparados por cidade</div>',unsafe_allow_html=True)
    ind=st.selectbox('Indicador',['Peso em aberto','Registros em aberto','Tempo médio de espera','Ocorrências'])
    gm=f.groupby('CIDADE',dropna=True).agg(PESO_ABERTO=('PESO',lambda s:s[~f.loc[s.index,'ENTREGUE']].sum()),REGISTROS_ABERTOS=('ENTREGUE',lambda s:(~s).sum()),TEMPO_MEDIO=('DIAS_ESPERA','mean'),OCORRENCIAS=('TEM_OCORRENCIA','sum')).reset_index(); key={'Peso em aberto':'PESO_ABERTO','Registros em aberto':'REGISTROS_ABERTOS','Tempo médio de espera':'TEMPO_MEDIO','Ocorrências':'OCORRENCIAS'}[ind]
    st.info('A camada visual do mapa ficará idêntica à Supervisão RCA assim que reutilizarmos a mesma base de coordenadas daquele projeto. Nesta V1, os dados logísticos já estão agregados por cidade e prontos para essa conexão.')
    st.dataframe(gm.sort_values(key,ascending=False).head(30),use_container_width=True,hide_index=True)
    ...
    st.dataframe(f[['DATA_IMPLANTACAO','CLIENTE','VENDEDOR','CIDADE','REGIAO','PRODUTO','VOLUMES','PESO','DIAS_ESPERA','MOTORISTA','CAMINHAO','STATUS','CLASSIFICACAO_ROTA','PREVISAO_ROTA','OBSERVACOES']].sort_values('DIAS_ESPERA',ascending=False),use_container_width=True,hide_index=True)

elif page=='Rotas e Entregas':
    a=f[~f.ENTREGUE]; cards([('Rotas',br(a.CLASSIFICACAO_ROTA.nunique()),''),('Clientes em aberto',br(a.CLIENTE.nunique()),''),('Peso em aberto',f'{br(a.PESO.sum()/1000,1)} t',''),('Em rota',br(f.EM_ROTA.sum()),''),('Sem previsão',br(a.PREVISAO_ROTA.isna().sum()),'')]); g=a.groupby('CLASSIFICACAO_ROTA',dropna=False).agg(PESO=('PESO','sum'),CLIENTES=('CLIENTE','nunique'),REGISTROS=('CLIENTE','size'),ESPERA=('DIAS_ESPERA','mean')).reset_index(); st.plotly_chart(px.bar(g,x='CLASSIFICACAO_ROTA',y='PESO',hover_data=['CLIENTES','REGISTROS','ESPERA']),use_container_width=True); st.dataframe(g.sort_values('PESO',ascending=False),use_container_width=True,hide_index=True)
    ...
    p=f[~f.ENTREGUE].copy(); p['SEM_PREVISAO']=p.PREVISAO_ROTA.isna(); p['SEM_MOTORISTA']=p.MOTORISTA.isna(); p['SEM_CAMINHAO']=p.CAMINHAO.isna(); p['OCORR_SEM_FOLLOW']=p.CATEGORIA_OCORRENCIA.notna()&p.FOLLOW_UP.isna(); p['PRIORIDADE']=np.where(p.DIAS_ESPERA.ge(10)|p.OCORR_SEM_FOLLOW,'CRÍTICA',np.where(p.DIAS_ESPERA.ge(5),'ATENÇÃO','NORMAL')); cards([('Pendências abertas',br(len(p)),''),('Espera ≥ 10 dias',br(p.DIAS_ESPERA.ge(10).sum()),''),('Sem previsão',br(p.SEM_PREVISAO.sum()),''),('Sem motorista',br(p.SEM_MOTORISTA.sum()),''),('Ocorr. sem follow-up',br(p.OCORR_SEM_FOLLOW.sum()),'')]); st.dataframe(p.sort_values('DIAS_ESPERA',ascending=False)[['PRIORIDADE','CLIENTE','CIDADE','VENDEDOR','PESO','DIAS_ESPERA','STATUS','PREVISAO_ROTA','MOTORISTA','CAMINHAO','CATEGORIA_OCORRENCIA','FOLLOW_UP']],use_container_width=True,hide_index=True)

elif page=='Ocorrências':
    o=f[f.TEM_OCORRENCIA].copy(); cards([('Ocorrências',br(len(o)),''),('Categorias',br(o.CATEGORIA_OCORRENCIA.nunique()),''),('Com follow-up',br(o.FOLLOW_UP.notna().sum()),''),('% com follow-up',f'{br(100*o.FOLLOW_UP.notna().mean(),1)}%','')]); g=o.groupby('CATEGORIA_OCORRENCIA',dropna=False).size().reset_index(name='QTD'); st.plotly_chart(px.bar(g,x='QTD',y='CATEGORIA_OCORRENCIA',orientation='h'),use_container_width=True); st.dataframe(o[['CLIENTE','CIDADE','MOTORISTA','CAMINHAO','CATEGORIA_OCORRENCIA','FOLLOW_UP','CONFERENTE','SEPARADOR','STATUS']],use_container_width=True,hide_index=True)

elif page=='Custos':
    c=f.CUSTOS.sum(min_count=1); p=f.PESO.sum(min_count=1); cards([('Custo informado',money(c if pd.notna(c) else 0),''),('Registros com custo',br(f.CUSTOS.notna().sum()),'cobertura parcial'),('Peso total',f'{br(p/1000,1)} t',''),('Custo/kg',money(c/p if pd.notna(c) and p else 0),'exploratório')]); st.warning('A V1 usa somente custos efetivamente informados na base.'); g=f[f.CUSTOS.notna()].groupby('CLASSIFICACAO_ROTA',dropna=False).agg(CUSTO=('CUSTOS','sum'),PESO=('PESO','sum'),REGISTROS=('CLIENTE','size')).reset_index(); g['CUSTO_KG']=g.CUSTO/g.PESO.replace(0,np.nan); st.dataframe(g.sort_values('CUSTO',ascending=False),use_container_width=True,hide_index=True)

st.markdown('---'); st.caption('V1 · processamento somente em memória · nenhuma alteração nos arquivos de origem.')
