...
UF_NOME={'AL':'Alagoas','BA':'Bahia','CE':'Ceará','MA':'Maranhão','PB':'Paraíba','PE':'Pernambuco','PI':'Piauí','RN':'Rio Grande do Norte','SE':'Sergipe'}
NOME_UF={unicodedata.normalize('NFKD',v).encode('ascii','ignore').decode('ascii').upper():k for k,v in UF_NOME.items()}
...
def load():
    src=source_bytes()
    raw=pd.read_excel(src,sheet_name='FORMAÇÃO 2026',header=1,engine='openpyxl')
    ...
    for c in ['CLIENTE','VENDEDOR','CIDADE','LOCAL_CARREGAMENTO','PRODUTO','CAMINHAO','MOTORISTA','STATUS','CLASSIFICACAO_ROTA','CATEGORIA_OCORRENCIA','FOLLOW_UP','CONFERENTE','SEPARADOR','OBSERVACOES']:
        d[c]=d[c].map(txt)
    ...
    try:
        ...
        if mc and rc:
            rg=rg[[mc,rc]].dropna(); rg['MUN_KEY']=rg[mc].map(city_key)
            d=d.merge(rg[['MUN_KEY',rc]].drop_duplicates('MUN_KEY'),on='MUN_KEY',how='left').rename(columns={rc:'REGIAO'})
...
def prepare_geo(base,gj):
    ...
    for ft in gj.get('features',[]):
        ...
        mun=city_key(p.get('NM_MUN')); uf=str(p.get('SIGLA_UF','')).upper().strip()
        loc=(mun or '')+'|'+uf; p['LOC']=loc; valid_locs.add(loc)
        if mun and uf: uf_by_city.setdefault(mun,set()).add(uf)
    b=base.copy()
    missing=b['UF_KEY'].isna()|b['UF_KEY'].astype(str).str.strip().eq('')
    b.loc[missing,'UF_KEY']=b.loc[missing,'MUN_KEY'].map(lambda m: next(iter(uf_by_city.get(m,set()))) if len(uf_by_city.get(m,set()))==1 else None)
...
def cidade_agg(base):
    ...
    g=x.groupby(['MUN_KEY','UF_KEY','UF_NOME'],dropna=False).agg(CIDADE=('CIDADE','first'),REGIAO=('REGIAO_OPERACIONAL','first'),PESO_ABERTO=('PESO_ABERTO_L','sum'),REGISTROS_ABERTOS=('ABERTO_L','sum'),CLIENTES=('CLIENTE','nunique'),TEMPO_MEDIO=('DIAS_ESPERA','mean'),OCORRENCIAS=('TEM_OCORRENCIA','sum'),EM_ROTA=('EM_ROTA','sum')).reset_index()
...
def mapa_logistico(base,key_prefix='mapa'):
    ...
    if reg not in ['Todas']+regions: reg='Todas'
    ...
    with c2:
        ...
        g_reg=g_estado if reg=='Todas' else g_estado[g_estado.REGIAO.eq(reg)]
        city_opts=sorted([x for x in g_reg.CIDADE.dropna().unique() if str(x).strip()])
        if st.session_state.get(key_prefix+'_city') not in ['Visão da região']+city_opts:
        ...
        if estado=='Nordeste' and reg=='Todas': titulo='Nordeste'
    ...
    if cidade_final!='Visão da região':
        ...
        if len(det_cli):
            resumo=det_cli.groupby(['CLIENTE','VENDEDOR'],dropna=False).agg(PESO=('PESO','sum'),VOLUMES=('VOLUMES','sum'),FORMACOES=('CLIENTE','size'),ESPERA_MEDIA=('DIAS_ESPERA','mean'),EM_ROTA=('EM_ROTA','sum'),OCORRENCIAS=('TEM_OCORRENCIA','sum')).reset_index().sort_values('PESO',ascending=False)
            st.dataframe(resumo,use_container_width=True,hide_index=True,column_config={
                'CLIENTE':'Cliente','VENDEDOR':'Vendedor','PESO':st.column_config.NumberColumn('Peso',format='%.0f kg'),'VOLUMES':st.column_config.NumberColumn('Volumes',format='%.0f'),'FORMACOES':'Formações','ESPERA_MEDIA':st.column_config.NumberColumn('Espera média',format='%.1f dias'),'EM_ROTA':'Em rota','OCORRENCIAS':'Ocorrências'
            })
            with st.expander('Ver detalhes das formações dos clientes',expanded=False):
                cols=['CLIENTE','VENDEDOR','PRODUTO','PESO','VOLUMES','DIAS_ESPERA','STATUS','PREVISAO_ROTA','MOTORISTA','CAMINHAO','CATEGORIA_OCORRENCIA','FOLLOW_UP']
...
if period and len(period)==2:
    a,b=pd.Timestamp(period[0]),pd.Timestamp(period[1])+pd.Timedelta(days=1)-pd.Timedelta(seconds=1)
...
if page=='Visão Geral':
    aberto=f[~f.ENTREGUE]
    cards([('Registros em aberto',br(len(aberto)),''),('Peso em aberto',f'{br(aberto.PESO.sum()/1000,1)} t',''),('Em rota',br(f.EM_ROTA.sum()),''),('Entregues',br(f.ENTREGUE.sum()),''),('Espera média',f'{br(aberto.DIAS_ESPERA.mean(),1)} dias',''),('Ocorrências',br(f.TEM_OCORRENCIA.sum()),'')])
    c1,c2=st.columns([1.2,1])
    with c1:
        g=aberto.groupby('REGIAO_OPERACIONAL',dropna=False).PESO.sum().reset_index().sort_values('PESO').tail(12)
        st.plotly_chart(px.bar(g,x='PESO',y='REGIAO_OPERACIONAL',orientation='h'),use_container_width=True)
    with c2:
        sit=pd.DataFrame({'Situação':['Entregue','Em rota','Carregado','Demais'],'Qtd':[f.ENTREGUE.sum(),f.EM_ROTA.sum(),f.CARREGADO.sum(),(~(f.ENTREGUE|f.EM_ROTA|f.CARREGADO)).sum()]})
    ...
    cards([('Registros',br(len(f)),''),('Clientes',br(f.CLIENTE.nunique()),''),('Volumes',br(f.VOLUMES.sum()),''),('Peso',f'{br(f.PESO.sum()/1000,1)} t',''),('Espera média',f'{br(f.DIAS_ESPERA.mean(),1)} dias','')])
    st.dataframe(f[['DATA_IMPLANTACAO','CLIENTE','VENDEDOR','CIDADE','REGIAO_OPERACIONAL','PRODUTO','VOLUMES','PESO','DIAS_ESPERA','MOTORISTA','CAMINHAO','STATUS','PREVISAO_ROTA']].sort_values('DIAS_ESPERA',ascending=False),use_container_width=True,hide_index=True)
elif page=='Rotas e Entregas':
    a=f[~f.ENTREGUE]
    cards([('Regiões',br(a.REGIAO_OPERACIONAL.nunique()),''),('Clientes em aberto',br(a.CLIENTE.nunique()),''),('Peso em aberto',f'{br(a.PESO.sum()/1000,1)} t',''),('Em rota',br(f.EM_ROTA.sum()),''),('Sem previsão',br(a.PREVISAO_ROTA.isna().sum()),'')])
    mapa_logistico(f,'rotas')
elif page=='Motoristas e Frota':
    m=f[f.MOTORISTA.notna()]
    g=m.groupby('MOTORISTA').agg(REGISTROS=('CLIENTE','size'),CLIENTES=('CLIENTE','nunique'),PESO=('PESO','sum'),ENTREGUES=('ENTREGUE','sum'),OCORRENCIAS=('TEM_OCORRENCIA','sum')).reset_index()
    cards([('Motoristas',br(m.MOTORISTA.nunique()),''),('Veículos',br(m.CAMINHAO.nunique()),''),('Peso associado',f'{br(m.PESO.sum()/1000,1)} t',''),('Em rota',br(m.EM_ROTA.sum()),''),('Ocorrências',br(m.TEM_OCORRENCIA.sum()),'')])
    st.dataframe(g.sort_values('PESO',ascending=False),use_container_width=True,hide_index=True)
elif page=='Pendências':
    p=f[~f.ENTREGUE].copy(); p['SEM_PREVISAO']=p.PREVISAO_ROTA.isna(); p['SEM_MOTORISTA']=p.MOTORISTA.isna(); p['OCORR_SEM_FOLLOW']=p.CATEGORIA_OCORRENCIA.notna()&p.FOLLOW_UP.isna()
    cards([('Pendências',br(len(p)),''),('Espera ≥10d',br(p.DIAS_ESPERA.ge(10).sum()),''),('Sem previsão',br(p.SEM_PREVISAO.sum()),''),('Sem motorista',br(p.SEM_MOTORISTA.sum()),''),('Ocorr. sem follow-up',br(p.OCORR_SEM_FOLLOW.sum()),'')])
    st.dataframe(p[['CLIENTE','CIDADE','VENDEDOR','PESO','DIAS_ESPERA','STATUS','PREVISAO_ROTA','MOTORISTA','CATEGORIA_OCORRENCIA','FOLLOW_UP']].sort_values('DIAS_ESPERA',ascending=False),use_container_width=True,hide_index=True)
elif page=='Ocorrências':
    o=f[f.TEM_OCORRENCIA].copy()
    cards([('Ocorrências',br(len(o)),''),('Categorias',br(o.CATEGORIA_OCORRENCIA.nunique()),''),('Com follow-up',br(o.FOLLOW_UP.notna().sum()),''),('% follow-up',f'{br(100*o.FOLLOW_UP.notna().mean(),1)}%','')])
    st.dataframe(o[['CLIENTE','CIDADE','MOTORISTA','CAMINHAO','CATEGORIA_OCORRENCIA','FOLLOW_UP','CONFERENTE','SEPARADOR','STATUS']],use_container_width=True,hide_index=True)
elif page=='Custos':
    custo=f.CUSTOS.sum(min_count=1); peso=f.PESO.sum(min_count=1)
    cards([('Custo informado',money(custo if pd.notna(custo) else 0),''),('Registros com custo',br(f.CUSTOS.notna().sum()),''),('Peso total',f'{br(peso/1000,1)} t',''),('Custo/kg',money((custo/peso) if pd.notna(custo) and peso else 0),'exploratório')])
